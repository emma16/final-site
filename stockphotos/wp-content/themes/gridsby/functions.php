...
function gridsby_scripts() {
	...
	if ( is_page_template( 'page-home.php' ) || is_page_template( 'page-gallery.php' ) || is_page_template( 'page-full-gallery.php' ) || is_category() && has_post_format( 'image' ) ) { 
...
function gridsby_posts_cat_gallery( $query ) {
    ...
    if ( $query->is_category() && has_post_format( 'image' ) || is_archive( 'download' ) ) {
        $query->set( 'posts_per_page', intval( $galnumber ) );
    }
}
add_action( 'pre_get_posts', 'gridsby_posts_cat_gallery' );